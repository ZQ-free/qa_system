...
- [阶段二点五：前端页面测试](#阶段二点五前端页面测试mock-模式下可用)
...
uvicorn main:app + 浏览器  →  测完整 HTTP 链路，适合验收和前端对接联调
```
...
pip install aiofiles   # 前端静态文件托管需要
```
...
## 阶段二点五：前端页面测试（Mock 模式下可用）

> **适用时机**：阶段二通过后，在接入真实数据库之前验证前端交互与后端接口的完整对接。
> 不依赖 Neo4j、MySQL 或 LLM，使用 Mock 假数据即可完整走通前端流程。

### 前置条件

- `.env` 中 `MOCK_MODE=true`
- `app/static/index.html` 文件存在
- 服务已用 `uvicorn main:app --reload --port 8000` 启动
...
正常应看到对话页面（左侧栏 + 右侧聊天区）。如果显示 404，说明 `app/static/index.html` 不存在，检查文件路径。
...
**预期**：
- 左侧栏显示"快速提问"按钮列表
- 右侧主区域显示欢迎文字和推荐问题芯片按钮
- 顶栏状态点为绿色（在线）
- 发送按钮为灰色禁用状态（输入框为空时不可点击）

---

### 测试用例2：基本问答流程

**操作**：在输入框输入 `青花瓷现在藏在哪个博物馆？`，按 Enter 或点击发送按钮

**预期**：
1. 用户消息出现在右侧气泡中
2. 助手侧出现三个跳动圆点（加载动画）
3. 加载动画消失，替换为回答文字
4. 回答气泡底部出现"数据来源"区域，显示博物馆名称标签
5. 出现"👍 有帮助"和"👎 不准确"两个反馈按钮
6. 输入框下方出现推荐追问按钮（如"青花瓷是什么朝代的？"）

> Mock 模式下回答内容是假数据，只需验证**格式正确、字段存在**即可，不验证内容准确性。

---

### 测试用例3：暂无数据响应

**操作**：输入 `这件文物不存在xyz123的材质是什么？`，发送

**预期**：
- 助手回答包含"暂无相关数据"字样
- `sources` 为空，不显示博物馆来源标签
- 不显示"部分内容由AI生成"标注
...
**操作**：点击左侧栏任意一个"快速提问"按钮

**预期**：
- 输入框自动填入对应问题文字
- 立即发送，走完整的问答流程（同测试用例2）

---

### 测试用例5：推荐追问按钮

**操作**：完成一次问答后，点击回答下方出现的推荐追问按钮

**预期**：
- 点击后自动发送对应问题，触发新一轮对话
- 旧的推荐追问消失，新回答出现后可能出现新的推荐追问
...
- 按钮变为高亮选中状态（蓝色边框）
- 两个反馈按钮均变为禁用，不可重复点击
- 浏览器控制台（F12 → Console）可能出现 `[反馈失败]` 警告，**这是正常的**，因为后端 feedback 接口暂未完整实现，不影响主流程

---

### 测试用例9：接口返回字段验证（开发者用）

打开浏览器开发者工具（F12），切换到 Network 标签，发送任意问题后找到 `/api/qa/ask` 请求，检查 Response 包含以下字段：

| 字段 | 类型 | 说明 |
|------|------|------|
| `answer_id` | string | 非空字符串 |
| `question` | string | 与发送的问题一致 |
| `answer` | string | 非空，回答文字 |
| `intent` | string | 如 `artifact_location` |
| `entity` | string | 识别出的实体名 |
| `sources` | array | 可为空数组 |
| `has_kg_facts` | bool | true 或 false |
| `has_llm_content` | bool | Mock模式下应为 false |
| `not_found` | bool | 找到数据时为 false |

**每个字段都存在且类型正确即为通过**，Mock 模式下不验证具体内容。
...
| 欢迎页显示 | 页面正常渲染，无报错 |
| 基本问答流程 | 能收到回答，来源标签可见 |
| 暂无数据响应 | 显示"暂无相关数据"，sources 为空 |
| 快捷按钮触发 | 点击后自动完成问答 |
| 推荐追问 | 点击后触发新对话 |
| 字数限制 | 不超过300字 |
| 清空对话 | 欢迎页重新出现 |
| 反馈按钮 | 按钮高亮，控制台警告不影响主流程 |
| 字段验证 | Network 面板中所有字段存在 |
...
**第三步**：重启服务，在前端页面发同样的问题

正常结果变化：回答从格式化的事实文本变成自然流畅的语言，Network 面板中 `has_llm_content` 变为 `true`：
...
用 CSV 数据里实际存在的文物名（如 `Jade Implement`）直接在前端输入框提问，验证：
- 回答内容与数据库中实际数据一致
- Network 面板中 `sources[0].object_id` 与 CSV 里的一致（如 `204498`）
- `sources[0].detail_url` 可以在新标签页正常访问
...
| 前端页面打开显示 404 | `app/static/index.html` 不存在 | 确认文件路径，或检查 `main.py` 中 `directory` 参数是否正确 |
| 前端发请求后控制台报 CORS 错误 | 前端和后端端口不一致 | 前端通过 `http://localhost:8000` 访问，不要用其他端口打开 HTML |
| 反馈按钮点击后控制台显示 `[反馈失败]` | feedback 后端接口暂未完整实现 | 正常现象，不影响主流程，待后续补充实现 |
...
| 服务健康检查 | 返回 `{"status":"ok"}` |
| 正常问题（curl） | `not_found=false`，`sources` 非空，有 `detail_url` |
| 不存在的文物 | `not_found=true`，`answer` 含"暂无相关数据" |
| 无关问题 | `intent=unknown`，`not_found=true` |
| **前端欢迎页** | **页面正常渲染，发送按钮灰色** |
| **前端问答流程** | **能收到回答，来源标签可见，反馈按钮出现** |
| **前端字段验证** | **Network 面板 9 个字段全部存在** |
